...
/* ---------------- Robot Movement ---------------- */
void move_robot(float linear, float angular, int seconds)
{
    char cmd[1024];
    int iterations = seconds * 5; // 5 Hz

    printf("Moving robot: linear=%.2f angular=%.2f for %d seconds\n", linear, angular, seconds);
    ...
    printf("Move complete, robot stopped.\n");
}
...
void process_move(const char *dir)
{
    ...
    if (strcmp(dir, "forward") == 0) linear = 0.1;
    else if (strcmp(dir, "backward") == 0) linear = -0.1;
    else if (strcmp(dir, "left") == 0) angular = 0.5;
    else if (strcmp(dir, "right") == 0) angular = -0.5;
    else {
        printf("Invalid direction: %s\n", dir);
        return;
    }

    move_robot(linear, angular, 3); // 3 seconds
}
...
Move* dequeue_move()
{
    pthread_mutex_lock(&lock);
    Move *m = head;
    if (m) { head = head->next; if (!head) tail = NULL; }
    pthread_mutex_unlock(&lock);
    return m;
}
...
int main()
{
    ...
    if (!daemon) { printf("Failed to start HTTPS server\n"); return 1; }
