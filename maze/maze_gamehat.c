...
static bool astar_running = false; // track if A* is running
...
static void turn_left_and_update(void) { send_pupper_move("left"); SDL_Delay(250); robot_heading = (robot_heading + 3) % 4; }
static void turn_right_and_update(void) { send_pupper_move("right"); SDL_Delay(250); robot_heading = (robot_heading + 1) % 4; }
static void move_forward_step(void) { send_pupper_move("forward"); SDL_Delay(250); }
...
static bool try_move(int *px, int *py, int dx, int dy) {
    ...
    if((dx==0 && dy==-1 && (w&WALL_N)) ||
       (dx==1 && dy==0 && (w&WALL_E)) ||
       (dx==0 && dy==1 && (w&WALL_S)) ||
       (dx==-1 && dy==0 && (w&WALL_W))) return false;
    *px=nx; *py=ny;
    return true;
}
...
static int astar(Node path[], int sx, int sy) {
    ...
    ANode nodes[MAZE_W*MAZE_H]; int open_list[MAZE_W*MAZE_H], open_count=0;
    for(int i=0;i<MAZE_W*MAZE_H;i++){ nodes[i].g=1000000; nodes[i].f=1000000; nodes[i].parent=-1; nodes[i].open=false; nodes[i].closed=false; }
    int start = sy*MAZE_W+sx, goal=(MAZE_H-1)*MAZE_W+(MAZE_W-1);
    nodes[start].g=0; nodes[start].f=heuristic(sx,sy); nodes[start].open=true;
    open_list[open_count++]=start;
    int dx[4]={0,1,0,-1}, dy[4]={-1,0,1,0}, masks[4]={WALL_N,WALL_E,WALL_S,WALL_W};

    while(open_count>0){
        int best=0; for(int i=1;i<open_count;i++) if(nodes[open_list[i]].f<nodes[open_list[best]].f) best=i;
        int current=open_list[best];
        open_list[best]=open_list[--open_count]; nodes[current].open=false;
        ...
        for(int d=0;d<4;d++){
            ...
            if(!nodes[ni].open || ng<nodes[ni].g){ nodes[ni].g=ng; nodes[ni].f=ng+heuristic(nx,ny); nodes[ni].parent=current;
...
static void draw(SDL_Renderer *r,int px,int py){
    SDL_SetRenderDrawColor(r,15,15,18,255); SDL_RenderClear(r);
    SDL_SetRenderDrawColor(r,230,230,230,255);
    for(int y=0;y<MAZE_H;y++)
        for(int x=0;x<MAZE_W;x++){
            int ox=PAD+x*CELL, oy=PAD+y*CELL, ex=ox+CELL, ey=oy+CELL; uint8_t w=g[y][x].walls;
    ...
    SDL_SetRenderDrawColor(r,40,160,70,255); SDL_RenderFillRect(r,&goal);
    SDL_Rect player={PAD+px*CELL+8,PAD+py*CELL+8,CELL-16,CELL-16};
    SDL_SetRenderDrawColor(r,255,215,0,255); SDL_RenderFillRect(r,&player);
    SDL_RenderPresent(r);
}
...
int main(void){
    ...
    SDL_Window *win=SDL_CreateWindow("Maze + Mini Pupper Control", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    ...
    int px=0, py=0; bool running=true;

    Node path[MAZE_W*MAZE_H]; int path_len=0, path_idx=0;
    ...
    while(running){
        ...
        while(SDL_PollEvent(&e)){
            ...
            if(e.type==SDL_KEYDOWN){
                ...
                // regenerate maze
                if(k==SDLK_r){ maze_init(); maze_generate(0,0); px=0; py=0; move_sequence=0; robot_heading=DIR_NORTH; astar_running=false; }
                ...
                if(k==SDLK_SPACE) send_pupper_move("stop");
            }
        }
        ...
        if(astar_running && path_idx<path_len){
            int oldx=px, oldy=py;
            px=path[path_idx].x; py=path[path_idx].y;
            orient_and_move_forward(oldx,oldy,px,py);
            move_sequence++;
            draw(r,px,py);
            SDL_Delay(3000); // 3s delay for A* step
            path_idx++;
